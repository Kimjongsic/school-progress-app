...
function initEvents() {
  ...
  document.getElementById('btnAddPeriod')?.addEventListener('click', () => { if(state.maxPeriods < 15) { state.maxPeriods++; renderSetupGrid('setup', true); }});
  document.getElementById('btnRemovePeriod')?.addEventListener('click', () => { if(state.maxPeriods > 1) { state.maxPeriods--; renderSetupGrid('setup', true); }});
  
  // [Edit 전용 이벤트]
  document.getElementById('btnMenuEditTime')?.addEventListener('click', openEditTimetable);
  document.getElementById('btnAddPeriodEdit')?.addEventListener('click', () => { if(state.maxPeriods < 15) { state.maxPeriods++; renderSetupGrid('edit', true); }});
  document.getElementById('btnRemovePeriodEdit')?.addEventListener('click', () => { if(state.maxPeriods > 1) { state.maxPeriods--; renderSetupGrid('edit', true); }});
  document.getElementById('btnSaveEditedTimetable')?.addEventListener('click', saveEditedTimetable);
...
async function saveEditedTimetable() {
  ...
  try {
    ...
    alert('시간표가 수정되었습니다.'); initApp();
  } catch (err) { alert('저장 실패'); btn.disabled = false; btn.innerText = "수정 완료"; }
}
...
function renderSetupGrid(viewType, keepValues = false) {
  ...
  for (let p = 1; p <= state.maxPeriods; p++) {
    ...
    row.innerHTML = `<td class="header-cell text-center font-bold text-slate-300 text-[10px] w-6">${p}</td>` + 
      ['월','화','수','목','금'].map(d => `<td><button class="setup-in sub-cell" data-day="${d}" data-p="${p}">과목</button><button class="setup-in gc-cell" data-day="${d}" data-p="${p}">반</button></td>`).join('');
...
window.toggleTagEditMode = (mode) => { state.isTagEditMode = !state.isTagEditMode; document.getElementById(mode === 'edit' ? 'btnEditTagsEditView' : 'btnEditTagsStep4').innerText = state.isTagEditMode ? "완료" : "편집"; renderSetupGrid(mode, true); };
...
async function fetchTimetable() {
  ...
  let finalSchedule = basic.data || [];
  if (changes.data) {
    const cancelled = changes.data.filter(c => c.change_type === 'cancelled').map(c => c.period);
    finalSchedule = finalSchedule.filter(b => !cancelled.includes(b.period));
    finalSchedule = [...finalSchedule, ...changes.data.filter(c => c.change_type === 'added')].sort((a,b) => a.period - b.period);
  }
