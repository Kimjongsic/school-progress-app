...
function initEvents() {
  ...
  document.getElementById('btnMenuInquiry').onclick = () => { toggleSettings(false); document.getElementById('inquiryPopup').classList.remove('hidden'); };
...
async function handleFinalSignUpSubmit() {
  ...
  try {
    const { error: pErr } = await supabase.from('profiles').insert({ user_id: userId, name: name, pin_code: pin });
    if(pErr) throw pErr;
  ...
  } catch (err) { alert('DB 저장 실패: ' + err.message); btn.disabled = false; }
}

// [Edit] 전용 수정 페이지 로드 함수
...
async function saveEditedTimetable() {
  ...
  try {
    ...
    alert('시간표가 성공적으로 수정되었습니다.'); initApp();
  } catch (err) { alert('수정 저장 실패'); btn.disabled = false; btn.innerText = "수정 완료"; }
}
...
function renderSetupGrid(viewType, keepValues = false) {
  ...
  const subC = viewType === 'edit' ? 'editQuickSub' : 'quickSubSection';
  const gcC = viewType === 'edit' ? 'editQuickGc' : 'quickGcSection';
  ...
  for (let p = 1; p <= state.maxPeriods; p++) {
    ...
    row.innerHTML = `<td class="text-center font-bold text-slate-300 text-[10px] w-6 header-cell">${p}</td>` + 
  ...
  window.renderTags('sub', subC); window.renderTags('gc', gcC);
...
async function fetchTimetable() {
  ...
  let finalSchedule = [];
  if (basic.data) {
    const cancelled = changes.data?.filter(c => c.change_type === 'cancelled').map(c => c.period) || [];
    finalSchedule = basic.data.filter(b => !cancelled.includes(b.period));
  }
  if (changes.data) {
    const added = changes.data.filter(c => c.change_type === 'added');
    finalSchedule = [...finalSchedule, ...added].sort((a,b) => a.period - b.period);
  }
  ...
  const dashboardHTML = await Promise.all(finalSchedule.map(async (item) => {
    ...
    return `<div class="bg-white p-6 rounded-3xl border border-slate-50 shadow-sm flex flex-col gap-4 active:scale-95 transition-all"><div class="flex justify-between items-center"><div class="flex items-center gap-3" onclick='window.openInputSheet(${JSON.stringify(item)}, "${prev?.content || '첫 기록'}", ${JSON.stringify(today)})'><span class="text-xs font-black bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg uppercase">${item.period}교시</span><span class="px-3 py-1 rounded-full text-xs font-black text-white" style="background:${subColor}">${item.subject}</span><span class="px-2.5 py-0.5 rounded-full text-[10px] font-black bg-white border-2" style="color:${gcColor}; border-color:${gcColor}">${item.grade_class}</span></div><button onclick='window.openMoveSheet(${JSON.stringify(item)})' class="text-slate-200"><i class="fa-solid fa-arrow-right-arrow-left"></i></button></div><div class="bg-slate-50 p-4 rounded-xl space-y-2"><p class="text-[10px] font-black text-amber-500 uppercase tracking-widest">Last: ${prev?.content || '-'}</p><p class="text-xs font-bold text-slate-700">Today: ${today ? today.content : '<span class="text-slate-200 italic">입력 전입니다</span>'}</p></div></div>`;
  }));
...
async function saveProgress() {
  ...
  showView('loadingView');
  await supabase.from('lesson_records').upsert({ user_id: state.user.id, user_name: state.user.name, date: state.activeDate.toISOString().split('T')[0], period: state.selectedItem.period, grade_class: state.selectedItem.grade_class, subject: state.selectedItem.subject, content: content, note: note || '-' }, { onConflict: 'user_id, date, period, grade_class, subject' });
...
async function handleConfirmMove() {
  ...
  await supabase.from('lesson_changes').insert([
    { user_id: state.user.id, user_name: state.user.name, date: state.activeDate.toISOString().split('T')[0], period: state.selectedMoveItem.period, subject: state.selectedMoveItem.subject, grade_class: state.selectedMoveItem.grade_class, change_type: 'cancelled' },
...
function handleNextButton() {
  if (state.signUp.step === 1) {
    if (!document.getElementById('regName').value || !document.getElementById('regPin').value) return alert('모든 정보를 입력하세요.');
